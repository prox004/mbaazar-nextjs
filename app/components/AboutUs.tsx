...
export default function AboutUs() {
  ...
  return (
    <section
      ref={containerRef}
      id="about"
      className="relative py-20 px-10 sm:py-28 bg-white text-black w-full min-h-[120vh]"
    >
      <div className="w-full px-4 sm:px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-20 items-start">
          ...
          <div className="flex flex-col justify-center space-y-6 lg:sticky lg:top-[28vh] h-fit">
            <h2 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-zinc-700 leading-tight">
              Redefining<br></br><span className="text-red-600 text-6xl sm:text-8xl">Fast Fashion</span>
            </h2>
          ...
          <div className="relative w-full flex flex-col items-center pb-[30vh]">
            {features.map((feature, i) => {
              const targetScale = Math.max(0.88, 1 - (features.length - i - 1) * 0.04);

              // Custom shades of red from light to mid-dark
              const cardShade =
                i === 0 ? "bg-red-50 border border-red-50" :
                  i === 1 ? "bg-red-50 border border-red-100" :
                    i === 2 ? "bg-red-50 border border-red-200" :
                ...
                >
                  ...
                  <div className="relative z-10 space-y-6">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 rounded-2xl bg-white/60 backdrop-blur-sm flex items-center justify-center text-red-600 group-hover:bg-white group-hover:text-red-600 transition-all duration-300">
                        {feature.icon}
                      </div>

                      <h3 className="text-5xl font-black tracking-tight text-zinc-950">
                        {feature.title}
                      </h3>
                    </div>

                    <p className="max-w-[280px] text-sm leading-7 text-zinc-600">
                      {feature.description}
                    </p>
                  </div>

                  {/* Accent Line */}
                  <div className="relative z-10 w-16 h-[2px] bg-red-500" />
                </StickyCard_001>
              );
            })}
          </div>
